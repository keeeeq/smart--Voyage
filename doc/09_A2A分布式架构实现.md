# 第九章：A2A 分布式架构实现详解

## 学习目标

- 理解 SmartVoyage 的 MCP + A2A 分布式架构
- 掌握各服务组件的实现细节
- 学会启动和调试分布式系统

## 1. 架构概述

SmartVoyage 采用四层分布式架构：

```
                           用户请求
                              ↓
┌─────────────────────────────────────────────────────────────┐
│                    Streamlit 前端 (:8502)                    │
│                         纯 UI 展示                           │
└─────────────────────────────────────────────────────────────┘
                              ↓ HTTP
┌─────────────────────────────────────────────────────────────┐
│                    API 网关 (:8000)                          │
│         意图识别 (LLM) → Agent 路由 → 结果润色 (LLM)         │
└─────────────────────────────────────────────────────────────┘
                              ↓ A2A Protocol
┌─────────────────────────────────────────────────────────────┐
│                     A2A Agent 层                             │
│  ┌───────────────┐ ┌───────────────┐ ┌───────────────┐      │
│  │ 天气Agent     │ │ 票务Agent     │ │ 订票Agent     │      │
│  │ :5005         │ │ :5006         │ │ :5007         │      │
│  │ LLM生成SQL    │ │ LLM生成SQL    │ │ 调用票务Agent │      │
│  └───────────────┘ └───────────────┘ └───────────────┘      │
└─────────────────────────────────────────────────────────────┘
                              ↓ MCP Protocol
┌─────────────────────────────────────────────────────────────┐
│                     MCP Server 层                            │
│  ┌───────────────┐ ┌───────────────┐ ┌───────────────┐      │
│  │ 天气MCP       │ │ 票务MCP       │ │ 订票MCP       │      │
│  │ :8002         │ │ :8001         │ │ :8003         │      │
│  │ 执行SQL查询   │ │ 执行SQL查询   │ │ 模拟预定      │      │
│  └───────────────┘ └───────────────┘ └───────────────┘      │
└─────────────────────────────────────────────────────────────┘
                              ↓
                         MySQL 数据库
```

## 2. MCP Server 层

### 作用
- 封装数据库操作为 MCP 工具
- 提供标准化的工具调用接口
- 处理数据格式转换

### 实现文件

| 文件 | 端口 | 功能 |
|------|------|------|
| `mcp_server/mcp_weather_server.py` | 8002 | 天气数据查询 |
| `mcp_server/mcp_ticket_server.py` | 8001 | 票务数据查询 |
| `mcp_server/mcp_order_server.py` | 8003 | 票务预定操作 |

### 核心代码

```python
from mcp.server.fastmcp import FastMCP

# 创建 FastMCP 实例
ticket_mcp = FastMCP(
    name="TicketTools",
    host="127.0.0.1",
    port=8001
)

# 注册工具
@ticket_mcp.tool(
    name="query_tickets",
    description="查询票务数据，输入 SQL 语句"
)
def query_tickets(sql: str) -> str:
    return service.execute_query(sql)

# 启动服务（使用 streamable-http 传输）
ticket_mcp.run(transport="streamable-http")
```

### 数据库连接自动重连

```python
def _ensure_connection(self):
    """确保数据库连接可用"""
    if self.conn is None or not self.conn.is_connected():
        self._connect()
```

## 3. A2A Agent 层

### 作用
- 接收用户查询，生成 SQL
- 调用 MCP Server 执行查询
- 格式化返回结果

### 实现文件

| 文件 | 端口 | 依赖 |
|------|------|------|
| `a2a_server/weather_server.py` | 5005 | MCP天气 (:8002) |
| `a2a_server/ticket_server.py` | 5006 | MCP票务 (:8001) |
| `a2a_server/order_server.py` | 5007 | 票务Agent + MCP订票 |

### 核心代码

```python
from python_a2a import A2AServer, AgentCard, run_server

# 定义 Agent Card
agent_card = AgentCard(
    name="TicketQueryAssistant",
    description="票务查询助手",
    url="http://localhost:5006",
    skills=[
        AgentSkill(
            name="execute ticket query",
            description="执行票务查询"
        )
    ]
)

# 实现 A2A Server
class TicketQueryServer(A2AServer):
    def handle_task(self, task):
        # 1. 从 task 获取用户输入
        conversation = task.message["content"]["text"]
        
        # 2. LLM 生成 SQL
        sql = self.generate_sql(conversation)
        
        # 3. 调用 MCP 执行查询
        result = asyncio.run(get_tickets(sql))
        
        # 4. 返回结果
        task.artifacts = [{"parts": [{"text": result}]}]
        task.status = TaskStatus(state=TaskState.COMPLETED)
        return task

# 启动服务
run_server(server, host="127.0.0.1", port=5006)
```

### SQL 生成提示词

```python
SQL_PROMPT = ChatPromptTemplate.from_template("""
## 数据库表结构

### concert_ticket（演唱会表）
- artist: 艺人名称
- city: 城市
- venue: 场馆
- show_date: 演出日期（注意：不是 event_date）

## 规则
1. 输出类型 JSON：{"type": "concert"}
2. 输出 SQL：SELECT * FROM concert_ticket WHERE ...
""")
```

## 4. 客户端层

### Streamlit 前端 (`streamlit_app_a2a.py`)

```python
# 意图识别
intents, queries, _ = intent_recognize(user_input)

# 根据意图选择 Agent
for intent in intents:
    if intent == "weather":
        agent_name = "WeatherQueryAssistant"
    elif intent in ["train", "flight", "concert"]:
        agent_name = "TicketQueryAssistant"
    elif intent == "order":
        agent_name = "TicketOrderAssistant"
    
    # 调用 Agent
    result = await call_agent(agent_name, query)
```

### 命令行客户端 (`smart_voyage_main.py`)

提供命令行交互界面，逻辑与 Streamlit 类似。

## 5. 通信协议

### MCP 调用

```python
from mcp import ClientSession
from mcp.client.streamable_http import streamablehttp_client

async with streamablehttp_client("http://127.0.0.1:8001/mcp") as (read, write, _):
    async with ClientSession(read, write) as session:
        await session.initialize()
        result = await session.call_tool("query_tickets", {"sql": sql})
```

### A2A 调用

```python
from python_a2a import A2AClient, Message, Task

client = A2AClient("http://localhost:5006")
message = Message(content=TextContent(text=query), role=MessageRole.USER)
task = Task(id="task-xxx", message=message.to_dict())
result = await client.send_task_async(task)
```

## 6. 端口分配

| 服务 | 端口 | 协议 |
|------|------|------|
| MCP 票务 | 8001 | MCP/HTTP |
| MCP 天气 | 8002 | MCP/HTTP |
| MCP 订票 | 8003 | MCP/HTTP |
| A2A 天气 | 5005 | A2A/HTTP |
| A2A 票务 | 5006 | A2A/HTTP |
| A2A 订票 | 5007 | A2A/HTTP |
| **API 网关** | **8000** | **HTTP** |
| Streamlit | 8502 | HTTP |

## 7. 启动方式

### 方式1：一键启动脚本
```bash
start_a2a.bat
```

### 方式2：PyCharm 运行配置
选择 "🚀 启动全部服务" 运行配置

### 方式3：手动启动
```bash
# MCP 层（3个终端）
python mcp_server/mcp_weather_server.py
python mcp_server/mcp_ticket_server.py
python mcp_server/mcp_order_server.py

# A2A 层（3个终端）
python a2a_server/weather_server.py
python a2a_server/ticket_server.py
python a2a_server/order_server.py

# API 网关
python api_gateway.py

# 前端
streamlit run streamlit_app_a2a.py
```

## 8. 调试技巧

### 查看日志
每个服务都有 INFO 级别日志，显示：
- 收到的查询
- 生成的 SQL
- MCP 返回结果

### 常见问题

| 问题 | 原因 | 解决 |
|------|------|------|
| Unknown column | SQL 列名错误 | 检查提示词中的表结构定义 |
| MySQL Connection | 数据库断连 | 服务已有自动重连机制 |
| Agent 离线 | 服务未启动 | 检查对应端口的服务 |

## 知识点总结

| 概念 | 说明 |
|------|------|
| FastMCP | 快速创建 MCP Server 的库 |
| streamable-http | MCP 的 HTTP 传输方式 |
| A2AServer | python-a2a 的服务端基类 |
| AgentCard | Agent 的能力描述卡片 |
| handle_task | A2A 任务处理入口方法 |

## 下一步

尝试修改 Agent 的提示词，添加新的查询功能！

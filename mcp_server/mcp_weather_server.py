...
def create_weather_mcp_server():
    ...
    @weather_mcp.tool(
        name="query_weather",
        description="查询天气数据，输入 SQL，如 'SELECT * FROM weather_data WHERE city = \"北京\" AND fx_date = \"2026-01-12\"'"
    )
    ...
    logger.info(f"端口: 8002")
    
    # 运行服务器
    try:
        print("天气 MCP 服务器已启动: http://127.0.0.1:8002/mcp")
